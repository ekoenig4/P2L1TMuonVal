...
step_size = 1000
def lazy_iterate(filelist, **kwargs):
    for f, tree in filelist.items():
        yield ut.lazy({f:tree}, **kwargs)

gen_iter = lazy_iterate({ f:'genTree/L1GenTree' for f in filelist})
l1_iter = lazy_iterate({ f:'gmtTkMuonChecksTree/L1PhaseIITree' for f in filelist})

total_events = 0
for gen_tree, l1_tree in tqdm(zip( gen_iter, l1_iter ), total=len(filelist)):

    # total_events += len(l1_tree)
