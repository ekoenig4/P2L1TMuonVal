...
from L1Trigger.Phase2L1GMTNtuples.root_tools import format_histo, format_histo2d
from L1Trigger.Phase2L1GMTNtuples.awkward_tools import cumand, get_dr
...
histos = namespace(
    ...
    genelectron_count_nomuon = format_histo("n_gen_ele_nomuon", "Electrons with No Muons;N Gen Electrons;", 10, 0, 10),
...
gen_selections = [gen_muon_mask]
if getattr(cfg, 'gen_selection', None):
    for key, selection in cfg.gen_selection.items():
        print(f' ... ... applying {selection}')
        gen_selections.append( eval(selection)(gen_tree) )
    gen_selections = cumand(gen_selections)

gen_mask = gen_selections[-1]
genmuon_counts = [ ak.sum(selection, axis=1) for selection in gen_selections ]
gen_parts = gen_parts[gen_mask]

gen_barrel_mask = ( np.abs(gen_tree.partEta) < cfg.barrel_eta )[gen_mask]
gen_overlap_mask = (( np.abs(gen_tree.partEta) > cfg.barrel_eta ) & ( np.abs(gen_tree.partEta) < cfg.endcap_eta ))[gen_mask]
gen_endcap_mask = ( np.abs(gen_tree.partEta) > cfg.endcap_eta )[gen_mask]
...
mask_labels = ['total'] + list(cfg.gen_selection.keys())
for label, ngen in zip(mask_labels, genmuon_counts):
    histos[f'n_{label}'] = format_histo(f'n_{label}_gen_muons', "{label};N Gen Muons", 10, 0, 10)
    fill_th1(histos[f'n_{label}'], ngen)

fill_th1(histos.genmuon_count, genmuon_counts[-1])
fill_th1(histos.genelectron_count_nomuon, (ak.sum( (np.abs(gen_tree.partId) == 11)[genmuon_counts[-1] == 0],axis=1 )))
...
l1_selections = [l1_parts.pt > 0]
if getattr(cfg, 'l1_selection', None):
    for key, selection in cfg.l1_selection.items():
        print(f' ... ... applying {selection}')
        l1_selections.append( eval(selection)(l1_tree) )
    l1_selections = cumand(l1_selections)
    l1_mask = l1_selections[-1]

    l1_parts = l1_parts[l1_mask]
