...
names = ["EXAMPLE", "EXAMPLE_TIGHTREL",
         "EXAMPLE_MEDIUMREL", "EXAMPLE_LOOSEREL"]
colors = [kBlack, kAzure, kAzure+1, kAzure+2]
markers = [20, 21, 25, 25]
leglabels = [
    "No Iso", "Tight Rel tkIso (hw)", "Medium Rel tkIso (hw)", "Loose Rel tkIso (hw)"]
...
for i in range(0, len(names)):
    print(names[i])
    files[i] = TFile(filepath+'effi_example__' +
                     names[i]+'_DoubleMuon_GMTIso.root')
