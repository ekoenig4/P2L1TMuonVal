...
histoMatchPt = formatHisto("matchGen_"+branch+"_" +
                           name+"_Pt", name, bins, start, end, color)
histoMatchEta = formatHisto(
    "matchGen_"+branch+"_"+name+"_Eta", name, binsEta, -2.5, 2.5, color)
histoMatchPhi = formatHisto(
    "matchGen_"+branch+"_"+name+"_Phi", name, 100, -4, 4, color)

histo2DPtEta = formatHisto2D(
    "genMuon2DPtEta", "Gen Muon Pt vs Eta", bins, start, end, binsEta, -2.5, 2-5)
histoMatch2DPtEta = formatHisto2D(
    "matchGen_"+branch+"_"+name+"_2DPtEta", "Gen Muon Pt vs Eta", bins, start, end, binsEta, -2.5, 2-5)
histoMatch2DPtEtaNStubs2 = formatHisto2D(
    "matchGen_"+branch+"_"+name+"_2DPtEta_NStubs2", "Gen Muon Pt vs Eta", bins, start, end, binsEta, -2.5, 2-5)
histoMatch2DPtEtaNStubs3 = formatHisto2D(
    "matchGen_"+branch+"_"+name+"_2DPtEta_NStubs3", "Gen Muon Pt vs Eta", bins, start, end, binsEta, -2.5, 2-5)
histoMatch2DPtEtaNStubs4 = formatHisto2D(
    "matchGen_"+branch+"_"+name+"_2DPtEta_NStubs4", "Gen Muon Pt vs Eta", bins, start, end, binsEta, -2.5, 2-5)

histoMatchPtNStubs2 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Pt_NStubs2", name, bins, start, end, color)
histoMatchEtaNStubs2 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Eta_NStubs2", name, binsEta, -2.5, 2.5, color)
histoMatchPhiNStubs2 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Phi_NStubs2", name, 100, -4, 4, color)

histoMatchPtNStubs3 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Pt_NStubs3", name, bins, start, end, color)
histoMatchEtaNStubs3 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Eta_NStubs3", name, binsEta, -2.5, 2.5, color)
histoMatchPhiNStubs3 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Phi_NStubs3", name, 100, -4, 4, color)

histoMatchPtNStubs4 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Pt_NStubs4", name, bins, start, end, color)
histoMatchEtaNStubs4 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Eta_NStubs4", name, binsEta, -2.5, 2.5, color)
histoMatchPhiNStubs4 = formatHisto(
    "matchGen_"+branch+"_"+name+"_Phi_NStubs4", name, 100, -4, 4, color)

histoL1Pt = formatHisto("match_l1_"+branch+"_"+name +
                        "_Pt", name, bins, start, end, color)
histoL1Eta = formatHisto("match_l1_"+branch+"_"+name +
                         "_Eta", name, binsEta, -2.5, 2.5, color)
histoL1Phi = formatHisto("match_l1_"+branch+"_"+name +
                         "_Phi", "", 100, -4, 4, color)
histoL1Stubs = formatHisto("match_l1_"+branch+"_" +
                           name+"_Stubs", name, 10, 0, 10, color)

histoNotMatchedPt = formatHisto(
    "noMatchGen_"+branch+"_"+name+"_Pt", name, bins, start, end, color)
histoNotMatchedEta = formatHisto(
    "noMatchGen_"+branch+"_"+name+"_Eta", name, binsEta, -2.5, 2.5, color)

histoBestDeltaR = formatHisto(
    "bestDeltaR_"+branch+"_"+name, "best R", 100, 0, 1, color)

histoMatchCheck = formatHisto(
    "matchCheck_"+branch+"_"+name, "", 10, 0, 10, color)

histoRes = formatHisto("ptres_"+branch+"_"+name,
                       "ptl1-ptgen / pt gen", 100, -1, 1, color)

effiPt = formatHisto("effi_"+branch+"_"+name+"_Pt",
                     name, bins, start, end, color)
effiEta = formatHisto("effi_"+branch+"_"+name+"_Eta",
                      name, binsEta, -2.5, 2.5, color)

effiPtNStubs2 = formatHisto(
    "effi_"+branch+"_"+name+"_Pt_NStubs2", name, bins, start, end, color)
effiEtaNStubs2 = formatHisto(
    "effi_"+branch+"_"+name+"_Eta_NStubs2", name, binsEta, -2.5, 2.5, color)
...
for event in tree:
    ...
    for i in range(0, vectorGenPt.size()):
        ...
        if vectorGenPt.at(i) < 1:  # some cleaning or this takes forever...
            continue
        # some cleaning or this takes forever...
        if vectorGenPt.at(i) < minPt or vectorGenPt.at(i) > maxPt:
            continue
                    ...
                    if vectorPfCandsId.at(k) != 4:
                        ...
                        if dR > 0.02:
                            ...
                            if vectorPfCandsId.at(k) == 0:
                                sumIsoDR02With002VetoCharged += vectorPfCandsPt.at(
                                    k)
        ...
        if bestDeltaR < 0.1 and matchMuon != -1:
            ...
            if vectorIdLUTPt.at(matchMuon) == 0:
                ...
                # this is for low Ptt
                if vectorIdLUTQuality.at(matchMuon) < thresholdQual[binEta]:
                    qual = False

            findBin = vectorIdLUTPt.at(matchMuon) | (
                vectorIdLUTEta.at(matchMuon) << 4)
# if vectorIdLUTQuality.at(matchMuon)<NoCutsInGaps[findBin]:   # Hybrid
            # Michalis tune
            if vectorIdLUTQuality.at(matchMuon) < constant_lt_tpsID[findBin]:
            ...
            histoRes.Fill(
                (vectorPt.at(matchMuon)-vectorGenPt.at(i))/vectorGenPt.at(i))
            ...
            if (vectorStubs.at(matchMuon) >= 2):
                ...
                histoMatch2DPtEtaNStubs2.Fill(
                    vectorGenPt.at(i), vectorGenEta.at(i))
