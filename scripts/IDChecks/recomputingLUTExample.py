...
def plotHistosStubs(fileMB, fileDoubleMuGun, name, xtitle, maxY=1, logY=False, BIN=0):
    ...
    for i in range(0, histo.GetXaxis().GetNbins()):
        if Int != 0:
            ...
            if Inti != 0:
                frac = Inti/Int
                errfrac = frac * \
                    math.sqrt((errInti/Inti)*(errInti/Inti) +
                              (errInt/Int)*(errInt/Int))
        ...
        if Int2 != 0:
            ...
            if Int2i != 0:
                frac2 = Int2i/Int2
                errfrac2 = frac2 * \
                    math.sqrt((errInt2i/Int2i)*(errInt2i/Int2i) +
                              (errInt2/Int2)*(errInt2/Int2))
    ...
    print("DEFAULT:", value_default, " E:",
          evalueForDef, " BG?:", bgvalueForDef)
...
for ptBin in range(0, 3):
    for etaBin in range(0, 13):
        ...
        plotHistosStubs(fMB, fDoubleMuGun, "muonQuality_"+str(ptBin)+"_"+str(etaBin),
                        "Quality PT:"+str(ptBin)+", ETA:"+str(etaBin), 0.4, False, BINLUT)
...
for ptBin in range(0, 3):
    ...
    for etaBin in range(0, 13):
        ...
        if ptBin == 0:
            print("%d %d %0.2f %d %0.2f %d %0.2f %d %0.2f \n" % (etaBin, lt_tpsID_98[BINLUT], bg_lt_tpsID_98[BINLUT], lt_tpsID_95[
                  BINLUT], bg_lt_tpsID_95[BINLUT], lt_tpsID_90[BINLUT], bg_lt_tpsID_90[BINLUT], lt_tpsID_85[BINLUT], bg_lt_tpsID_85[BINLUT]))
    ...
    mg.Add(gr90)
    mg.Add(gr85)
    mg.SetTitle("Bg rejection for fixed efficiency; BIN ETA (PTBIN=" +
                str(ptBin)+"); Bg Rejection")
    mg.SetMaximum(1.1)
    c3 = TCanvas("c3")
    c3.cd()
    mg.Draw("ALPE")
